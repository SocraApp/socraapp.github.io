...
class MarkdownEditor {
    ...
    for (let i = 0; i < lineCount; i++) {
      ...
      if (fenceOpen && !inCodeBlock) {
        ...
        if (!isActive) {
          ...
          this._mark(i, 0, line.length, { css: 'font-size: 0; line-height: 0; opacity: 0; overflow: hidden' });
  ...
  _decorateLine(lineNum, line, isActive, cursorCh) {
    ...
    if (headingMatch) {
      ...
      if (!isActive) {
        this._mark(lineNum, 0, prefixLen, { css: 'font-size: 0; line-height: 0; opacity: 0; overflow: hidden' });
      }
    ...
    if (line.match(/^(-{3,}|\*{3,}|_{3,})\s*$/)) {
      if (!isActive) {
        this._mark(lineNum, 0, line.length, { css: 'font-size: 0; line-height: 0; opacity: 0; overflow: hidden' });
  ...
  _decorateInline(lineNum, line, isActive, cursorCh) {
    ...
    while (i < line.length) {
      ...
      if ((match = line.substring(i).match(/^\$\$([^$]+)\$\$/))) {
        ...
        if (!cursorInEl) {
          this._mark(lineNum, elStart, elEnd, { css: 'font-size: 0; line-height: 0; opacity: 0; overflow: hidden' });
          try {
      ...
      if ((match = line.substring(i).match(/^\?([^?\n]+)\?/))) {
        ...
        const cursorInEl = isActive && cursorCh > elStart && cursorCh <= elEnd;
        if (!cursorInEl) {
          this._mark(lineNum, elStart, elStart + 1, { css: 'font-size: 0; line-height: 0; opacity: 0; overflow: hidden' });
          this._mark(lineNum, elEnd - 1, elEnd, { css: 'font-size: 0; line-height: 0; opacity: 0; overflow: hidden' });
      ...
      if ((match = line.substring(i).match(/^\[([^\]]+)\]\(([^)]+)\)/))) {
        ...
        if (!cursorInEl) {
          this._mark(lineNum, elStart, elStart + 1, { css: 'font-size: 0; line-height: 0; opacity: 0; overflow: hidden' });
          this._mark(lineNum, elStart + 1 + match[1].length, elEnd, { css: 'font-size: 0; line-height: 0; opacity: 0; overflow: hidden' });
  ...
  _decorateSpan(lineNum, elStart, elEnd, delimLen, className, isActive, cursorCh) {
    const cursorInEl = isActive && cursorCh > elStart && cursorCh <= elEnd;
    if (!cursorInEl) {
      this._mark(lineNum, elStart, elStart + delimLen, { css: 'font-size: 0; line-height: 0; opacity: 0; overflow: hidden' });
      this._mark(lineNum, elEnd - delimLen, elEnd, { css: 'font-size: 0; line-height: 0; opacity: 0; overflow: hidden' });
    }
